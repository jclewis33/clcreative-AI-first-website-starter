...
**Why a `/preview` tree instead of drafts on the public URLs:** with `@astrojs/cloudflare`, the request handler returns a matching static asset **before** `app.render()` is ever called — Astro middleware never runs for a prerendered path, so a cookie-keyed draft rewrite on the public URL is impossible (including with `run_worker_first`). Serving the CMS routes as SSR just to enable preview is the alternative that was removed: it burned Worker CPU per request and caused real production 503s (`exceededCpu`) under crawler load. Do not regress to either approach.
...
Every public route is prerendered, so `@astrojs/sitemap` enumerates the whole site — **including the CMS content routes** — automatically from the route table. There is **no** `customPages` option and **no** `getSanityUrls()` helper; that workaround existed only for SSR routes (which are invisible to the sitemap) and must not come back. A new content type only needs a prerendered route with `getStaticPaths` and it appears in the sitemap on the next build.
...
- Re-introducing `sitemap({ customPages })` / a `getSanityUrls()` helper — prerendered routes are enumerated automatically; the workaround only existed for SSR routes
