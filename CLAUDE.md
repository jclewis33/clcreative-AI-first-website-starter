...
- **Never render a slot to test whether it is empty.** `Astro.slots.render()` (and any helper built on it) **swallows the hoisted `<script>` of every component inside that slot — for the whole page, not just that slot.** Measured: a single `<Slider>` placed in a Card's `footer` slot left *both* sliders on the page uninitialized, including one that was nowhere near a rendered slot. This shipped briefly as Lumos-style "self-erasure" on `<Section>` and took out every interactive component on the site (sliders, tabs, accordions, modals) with **no console error** — the scripts simply never arrive. Use `Astro.slots.has(name)` instead: it only reports whether a slot was *passed* (a slot holding an empty `{items.map(…)}` still counts as present), and a rare empty wrapper is a much cheaper bug than silently dead client JS. To skip a whole component when there is nothing to show, use the `render` prop — `<Section render={posts.length > 0}>` — which decides from data instead of from slot output. A second, separate reason not to render slots in **frontmatter**: it runs children before the image service initializes and crashes with `Cannot read properties of undefined (reading 'validateOptions')`.
