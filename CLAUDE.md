...
- **Never render a slot to test whether it is empty — use `:empty` in CSS instead.** `Astro.slots.render()` (and any helper built on it) **swallows the hoisted `<script>` of every component inside that slot — page-wide, not just that slot.** Measured: one `<Slider>` in a Card `footer` slot left *both* sliders on the page uninitialized, including one nowhere near a rendered slot. This shipped briefly as Lumos-style "self-erasure" on `<Section>` and took out every interactive component on the site (sliders, tabs, accordions, modals) with **no console error** — the scripts simply never arrive. Reproduced in Lumos' own repo on Astro 7 as well as here on Astro 6, so it is not a version bug: his `Section.astro` has the same latent flaw and only escapes it because his only two scripted components (Nav, Footer) never sit inside a Section slot.
  **The safe way to get the same result:** when a slot renders nothing, Astro emits the wrapper with *nothing at all* between the tags (verified in dev and in the production build — not even whitespace), so `:empty` matches exactly the empty case, and a wrapper holding a bare text node is correctly left alone. `<Section>` and Card's footer collapse themselves that way (`display: none`), which costs no slot render and cannot touch scripts. Use `Astro.slots.has(name)` when you need a JS-side check — it only reports whether a slot was *passed*. To drop markup from the HTML entirely rather than collapse it, decide from **data** at the call site: `<Section render={posts.length > 0}>`. A second, separate reason not to render slots in **frontmatter**: it runs children before the image service initializes and crashes with `Cannot read properties of undefined (reading 'validateOptions')`.
