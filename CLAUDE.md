...
- **Images**: Wrap images with `u-image-wrapper` (controls dimensions, radius, overflow) and apply `u-image` to the `<img>` element (absolute-fill with focal-point positioning via `--_x`/`--_y`). Use `is-background` on the wrapper for Section background slots.
...
- **Slots**: `<Layout>`'s default slot IS column 1 — put content straight inside it, no slot attribute. The second column is `slot="column2"`. A single component (Visual, Grid, Card) takes it directly. For **several elements in column 2 there are two equally valid forms**: repeat `slot="column2"` on each element (Astro collects them into the slot in source order), or group them in one `<Fragment slot="column2">`. These produce the _same_ children inside the column — verified in the rendered DOM. `<Fragment>` emits no element of its own; there is no `display: contents` wrapper involved (that was the old `<Col>` component, removed). The only difference: repeating the attribute leaves an inert `slot` attribute on each rendered element. Pick whichever reads better.
  A `<Visual>` may share a column with other content freely — it holds its `ratio` at every width (measured 1.778 for a 16/9 visual at 1440/700/480px in all four arrangements). It used to stretch once the layout collapsed; see the image-fill rule below.
- **Images: `height: auto` is the default, filling a box is opt-in.** `aspect-ratio` only computes a **missing** dimension. `.u-image-wrapper` already sets `width: 100%`, so the moment it also has a definite height the ratio has nothing left to decide and is silently ignored. The wrapper used to default to `height: 100%`, which was harmless while the parent's height was indefinite (a percentage against an auto-height parent resolves to `auto`) but stretched the image the moment the parent height became definite — e.g. a Layout column holding an image _and_ a caption once collapsed to one column, where a 16/9 visual measured **1.66** instead of 1.78. The base is now `height: auto` (`styles/base/visual-utilities.css`), matching Lumos, whose base is `img { height: auto }` with `height: 100%` opted into per pattern.
...
- **Never call `Astro.slots.render()` to find out whether a slot is empty. Use `:empty` in CSS.** Two independent failures, both silent, both measured:
  ...
  2. **It breaks images on a cold start.** Rendering a slot that contains `<Image>`/`<Visual>` runs those children before the image service is initialized: `Cannot read properties of undefined (reading 'validateOptions')`. The page still returns 200 — it just silently loses sections (homepage measured at **9 sections instead of 17**). Emitting the captured string with `set:html` instead of `<slot />` fixes failure 1 but _not_ this one, and moving the call from frontmatter into the template does not help either. Both were tried and reverted.
     **What to do instead.** Gate on `Astro.slots.has(name)` when you need a JS-side check — it only reports whether a slot was _passed_ (cheap, no render, no hazard). For the residual case where a slot is passed but renders nothing, let CSS erase it: Astro emits the wrapper with **nothing at all** between the tags (verified in dev and in the production build — not even whitespace), so `:empty` matches exactly the empty case, and a wrapper holding a bare text node is correctly left alone. `<Section>` collapses itself that way (`.u-section:has(> .u-container:empty)`), and `.card_primary_footer:empty` does the same for a card footer that would otherwise charge its `padding-top` and `margin-top: auto`. To drop markup from the HTML entirely rather than collapse it, decide from **data** at the call site: `<Section render={posts.length > 0}>` — no slot render, always safe.
     **Prefer a prop over a slot for plain text.** A string prop makes emptiness a frontmatter truthiness check with none of the above in play — this is how Lumos' `Card` avoids the problem entirely (it has _no_ slots: `eyebrow`, `heading`, `text`, `buttonLabel` are props, and it self-erases on `!!(eyebrow || heading || text || buttonLabel)`). Reach for a slot when the caller genuinely needs to pass arbitrary markup.
- **Exported types in `.astro` frontmatter: never start the type with a leading `|`.** Component props live in the component's own file (one file per component — there are no `.props.ts` modules). Two rules keep that working:
  1. `export type X =` whose **first token is a leading pipe** on the next line breaks the _build_ — `Unexpected "export"`, because Astro fails to hoist that declaration out of the component function. `astro check` passes, so only `npm run build` catches it. Verified minimal case: `export type A =\n  | "one"\n  | "two";` fails, while the same union written as `"one" | "two"` (wrapped or not) builds, and a **non-exported** leading-pipe union builds fine. A leading pipe _inside_ parentheses is also fine, which is why `export type Props = BaseProps & ( { … } | { … } )` works.
  2. Prettier only produces the dangerous shape when a union carries **per-member JSDoc**; without it, it wraps to the safe form. So an exported union must document its values in the prop's own JSDoc (a table), not comment-per-member.
     Practical consequence: **export only what another file imports** — that is `Props` (for `src/lib/prop-contracts.test-d.ts`) and `PaddingSize` (imported by every section wrapper). Every other helper type stays unexported, and can then be formatted any way Prettier likes.
- **Protecting the prop tooltips: run `npm run check:hover`.** Every prop carries JSDoc so hovering it in the editor explains what it does. That contract breaks in two ways that _nothing else notices_ — `astro check`, `npm run build` and a pixel diff all stay green while every tooltip in a file is silently gone, and invalid props stop erroring at the call site too. `scripts/check-hover.mjs` drives the real Astro language server over LSP and asserts that all 503 documented props still show their description on hover. Two known causes, both measured, both previously live in this repo:
  1. **A stray `<` or `>` in a frontmatter comment detaches the whole file's `Props` type at the call site.** Write comparisons in words — "posts.length is greater than 0" — never with the literal character. Balanced tag-like pairs inside `@example` blocks (`<Visual src={img} />`) are safe; it is the odd one out that breaks it. This had disabled tooltips _and_ call-site prop checking across `Button`, `Card`, `Layout`, `ScrollReveal` and `Visual` — and the comment that used to warn about it contained the very characters it warned about, which is how it spread to four files.
...
- **Renaming anything? Grep the old name before you finish.** A rename is the one edit that fails _silently_ — the old selector still parses, the old `var()` still resolves to its fallback, the old slot name is simply ignored. Nothing in `astro check`, the build, or a pixel diff catches it. This has bitten this project three times: a `--surface-*` doc that outlived the tier, `.pricing_card_element > .u-content-wrapper` left behind when ContentWrapper became Layout (which killed `margin-top: auto` on every pricing CTA), and `object-position: var(--x)` still reading the pre-`--_` name, which silently sent **every** `<Visual position={{…}}>` focal point back to dead centre.
...
- **Don't repeat the layout's alignment on the text.** `text-align` inherits, so inside `<Layout variant="stack-centered">` (or `card`, or any `.u-content-wrapper.is-center-align`) headings and paragraphs are already centred — `align="center"` on the child does nothing. Worse, it **hard-codes** the alignment: change the Layout variant later and that element stays centred while its siblings follow the layout. Let the layout own alignment. `align` is for the case where the surrounding context is _not_ aligned that way — e.g. an empty-state message inside a left-aligned results grid (`blog_no_results`). Measured when this was cleaned up: 14 redundant `align="center"` attributes removed, 594 text elements checked, zero changes to computed `text-align` or geometry.

- **The `.u-text` element — NOT a wrapper (retired Webflow shape).** `.u-text` sits directly on the `<h1>`–`<h6>`/`<p>`/`<span>` that `<Heading>` and `<Text>` render. Do **not** reintroduce a wrapper around text.

  Until this was removed, both components emitted `<div class="u-text u-text-style-h2"><h2>…</h2></div>`: a flex column with `min-width: 100%`, whose child then needed `max-width: inherit !important; margin-inline: 0 !important; margin-top: 0 !important` purely to undo the box it had just been put inside — three of the repo's `!important` declarations existing only to cancel a wrapper. It also made a flex box the direct parent of every text element, contradicting the rule directly above this one.

  It came from Webflow, where a Text Block cannot carry its own `max-width` or alignment independently of flow, so you wrap it. In CSS the element holds those properties itself.

  Two things the wrapper genuinely did, and where they live now:

  1. **Filling the column** so `text-align` positions the text rather than the box shrinking to its content — now `width: 100%` on `.u-text`.
  2. **Centring a `max-width`-constrained box.** Layout containers declare `align-items` as an _inheritance hint_ (they are block boxes, so it does nothing to their own children); the flex wrapper consumed it via `align-items: inherit` and did the centring. A block element cannot, and `text-align` only moves text _inside_ the box. That signal is now **`--_text-inline-margin`**, an inheriting custom property set alongside every `align-items` hint — `.u-content-wrapper.is-*-align`, and the `stack-centered` / `card` Layout variants. **If you add a new centred layout context, set `--_text-inline-margin: auto` there too, or constrained headings will silently left-align.**

  Removing it also fixed a real bug: inside `.u-rich-text`, headings got **double bottom spacing** — their own margin trapped inside the flex wrapper (which prevents collapsing) _plus_ the wrapper's own margin. Legal/prose pages are ~32px shorter as a result. Verified across 8 pages × 2 viewports: every production page is pixel-identical apart from that fix.

- **`<body>` is the page shell — no `.page_wrap` (retired Webflow shape).** BaseLayout renders Navbar, `<main>`, and Footer as **direct children of `<body>`**. Do not wrap them in a container div.

  Until this was removed, every page was `<body><div class="page_wrap">…</div></body>`, and the wrapper carried the entire shell: `display: flex; flex-direction: column; min-height: 100svh; overflow: clip`. That div is a Webflow artifact twice over — Webflow's canvas cannot style `<body>`, and its copy/paste model needs one selectable container you can duplicate onto another page. Astro has neither limit: the layout _is_ the reusable unit, so duplicate or import `BaseLayout` instead.

  Where the four declarations live now — all on `body` in [src/styles/reset.css](src/styles/reset.css), with `body > main { flex: 1 }` below it to push the footer down:

  1. **`display: flex; flex-direction: column; min-height: 100svh`** — unchanged, just one box up.
  2. **`overflow: clip` → `overflow-x: clip`, and the axis matters.** With `html`'s overflow left `visible`, the browser **propagates `body`'s overflow to the viewport**. Naming only `overflow-x` stops sideways scrolling (decorative art, marquees) while vertical scrolling stays normal. Writing the `overflow: clip` shorthand on `body` instead would propagate to **both** axes and freeze the page — no vertical scrolling at all. Keep it `clip`, never `hidden`: `clip` does not create a scroll container, which is the same reason the wrapper used it.

  The modal / mobile-nav scroll lock stays on `html` (`html:has(dialog.modal_dialog[open]) { overflow: hidden }`), where the shorthand _is_ correct and its `:has()` specificity beats the `body` rule.

  Verified: 5,469 matched elements across 8 routes compared on 34 computed properties — zero differences; 200 shell measurements across 10 pages × 2 viewports (document scroll extents, `main`/footer/nav geometry, real-wheel scrolling) — zero differences; plus a probe that an element overflowing 360px to the right still cannot be scrolled to, while the same probe with the clip disabled scrolls 360px.

- **Text parent containers**: Direct parents of text elements should not be `display: flex` — flex prevents vertical margin collapsing between text. Use `display: block` or no display override for text wrappers. (`<Heading>`/`<Text>` used to violate this themselves by wrapping every heading and paragraph in a flex `.u-text` div; that wrapper is gone — see **The `.u-text` element** below.) Add `u-margin-trim` to the direct parent of text elements with margins to prevent unwanted extra space at the edges.
...
This used to require a `<Col>` component (`<div class="u-display-contents">`). `<Col>` was merged into `<Layout>` — the default slot _is_ column 1 — so the wrapper is gone. If you find a leftover `<div class="u-display-contents">` around loose Layout children, delete it; check first whether any CSS hangs off its class, because `display: contents` wrappers are invisible in the layout but still perfectly good selector hooks (the home hero's staggered entrance was hung off one).
