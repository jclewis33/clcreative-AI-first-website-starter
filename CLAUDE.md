...
- **Testing whether a slot is empty: render it ONCE and emit the captured string — never render it and then also use `<slot />`.** Astro emits a component's hoisted `<script>` tag as *part of its rendered output*. So `await Astro.slots.render(name)` puts that tag inside the returned string; if you use the string only as a yes/no test and then render the children again through `<slot />`, the tag went in the bin and is **not** re-emitted — the markup appears, the script never arrives, and there is no error. Verified by A/B on two identical pages: the only difference in the emitted HTML was a missing `Slider.astro?astro&type=script` tag. This shipped briefly on `<Section>` and killed every slider, tab, accordion and modal on the site at once. Reproduced in Lumos' own repo on Astro 7 (his `Section.astro` + `utils/slots.ts` have the same flaw, latent — his only scripted components, Nav and Footer, never sit in a Section slot), so it is not version-specific and an Astro upgrade does not fix it.
  **The working pattern** is what `<Section>` now does: capture each slot once with `Astro.slots.render()`, gate on the captured string, and emit that string with `set:html` instead of `<slot />`. Children render exactly once, the script tag rides along inside the string, and a section with nothing to show emits nothing at all — no `<section>`, no container, no padding. Two constraints: capture in the **template**, never in frontmatter (frontmatter runs children before the image service initializes and crashes with `Cannot read properties of undefined (reading 'validateOptions')`), and remember `set:html` replaces `<slot />` rather than joining it. Where a wrapper still uses `Astro.slots.has()` (Card, Layout, the section heading slots) it only knows a slot was *passed*, so a passed-but-empty slot can leave a wrapper behind; Card's footer mops that up with `.card_primary_footer:empty { display: none }`. To skip markup from **data** instead, `render={posts.length > 0}` needs no slot render at all and is always safe.
