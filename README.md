...
- **In Claude Code:** run **`/setup`** — an orchestrator that takes you from a fresh fork to a deployed site. It scopes the project (including **whether you want Sanity as your CMS at all** — it's optional, and the repo builds and deploys fine without it), gathers your identity, **pulls your brand colors from Figma** (if a Figma MCP is connected), asks about your **fluid type scale**, walks you through swapping **fonts, the OG image, favicon, logos, and the placeholder photography**, rewrites the config files, runs the full CI gate, and hands you the remaining dashboard steps. If you do want Sanity, it also **creates the project + dataset, deploys the schema and Studio, and adds CORS origins**. ([skill](.claude/skills/setup/SKILL.md))
